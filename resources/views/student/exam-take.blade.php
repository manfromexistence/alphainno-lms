    ...
    <div class="max-w-7xl mx-auto px-4">
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
            ...
            <div class="lg:col-span-1">
                <!-- Timer -->
                <div class="bg-white rounded-lg shadow p-6 mb-4 sticky top-4">
                    ...
                    <div class="mt-4 pt-4 border-t">
                        <div class="flex justify-between text-sm mb-2">
                            ...
                            <span id="progress-text" class="font-semibold">0/{{ $total_questions }}</span>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-2 mt-3">
                            <div class="bg-green-50 border border-green-200 rounded p-2 text-center">
                                <div id="answered-count" class="text-xl font-bold text-green-600">0</div>
                                <div class="text-xs text-gray-600">Answered</div>
                            </div>
                            <div class="bg-orange-50 border border-orange-200 rounded p-2 text-center">
                                <div id="unanswered-count" class="text-xl font-bold text-orange-600">{{ $total_questions }}</div>
                ...
                <div class="bg-white rounded-lg shadow p-6">
                    ...
                    <div class="mt-4 pt-4 border-t space-y-2 text-xs">
            ...
            <div class="lg:col-span-3">
                ...
                <div class="bg-white rounded-lg shadow p-6 mb-6">
                    <div class="flex justify-between items-start">
                        <div>
                            <h1 class="text-2xl font-bold text-gray-900 mb-1">{{ $exam->title }}</h1>
                ...
                <form id="exam-form" method="POST" action="{{ route('student.exams.submit', $exam) }}">
                    ...
                    @foreach($questions as $index => $question)
                    <div id="question-{{ $index }}" class="question-panel bg-white rounded-lg shadow p-6 mb-6 {{ $index > 0 ? 'hidden' : '' }}">
                        
                        <!-- Question Header -->
                        <div class="flex justify-between items-center mb-6 pb-4 border-b">
                            <div class="flex items-center gap-3">
                                <span class="bg-primary text-white px-4 py-2 rounded-lg font-bold">
                                    Question {{ $index + 1 }}
                                </span>
                                <span class="text-gray-500">of {{ $total_questions }}</span>
                            </div>
                            <div class="flex items-center gap-2">
                                <svg class="w-5 h-5 text-yellow-500" fill="currentColor" viewBox="0 0 20 20">
                                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                                </svg>
                                <span class="font-semibold text-yellow-600">{{ $question->marks ?? 1 }} marks</span>
                            </div>
                        </div>
                        ...
                        <div class="space-y-3">
                            @foreach(['A', 'B', 'C', 'D'] as $optKey)
                                ...
                                @if($optionValue)
                                <label class="flex items-start p-4 bg-gray-50 rounded-lg cursor-pointer hover:bg-blue-50 border-2 border-gray-200 hover:border-primary transition option-label">
                                    <input type="radio" 
                                    ...
                                    <div class="ml-3 flex-1">
                                        <div class="flex items-start">
                                            <span class="inline-flex items-center justify-center w-8 h-8 bg-primary text-white font-bold rounded-lg mr-3 flex-shrink-0">
                                                {{ $optKey }}
                                            </span>
                                            <span class="text-gray-900 text-base leading-relaxed">
                                                {{ $optionValue }}
                                            </span>
                                        </div>
                                    </div>
                        ...
                        <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                            <p class="text-yellow-800">This question type is not supported in this view.</p>
                        </div>
                        @endif
                        ...
                        <div class="flex justify-between items-center mt-8 pt-6 border-t">
                            <button type="button" 
                                    ...
                                    class="flex items-center px-6 py-3 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 font-medium transition {{ $index === 0 ? 'invisible' : '' }}">
                            ...
                            <button type="submit" 
                                    class="flex items-center px-8 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 font-bold shadow transition">
                                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <button type="button" 
                                    onclick="goToQuestion({{ $index + 1 }})" 
                                    class="flex items-center px-6 py-3 bg-primary text-white rounded-lg hover:opacity-90 font-medium shadow transition">
                                Next
